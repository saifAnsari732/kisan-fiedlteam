...
// Global cached Mongo connection for Vercel serverless
...
async function getDbConnection(): Promise<typeof mongoose | null> {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(mongoUri.trim(), opts).then((m) => {
      ...
      // Pre-seed demo users in MongoDB in background
      (async () => {
        try {
...
export function createExpressApp() {
  ...
  router.get('/health', async (req, res) => {
    ...
    res.json({
      ...
      activeStore: isMongo ? 'MongoDB Atlas' : 'In-Memory Fallback Store',
    ...
    try {
      ...
      if (dbConn && mongoose.connection.readyState === 1) {
        try {
          ...
          console.warn('Mongo query failed, falling back to memory store:', mErr);
        }
      }
      ...
      if (!user) {
        return res.status(401).json({ error: 'Invalid credentials. User not found. Click "Create one" to register.' });
      }
  ...
  router.get('/auth/me', async (req, res) => {
    try {
      ...
      if (!user) {
        user = inMemoryStore.users.find((u) => u._id === userId);
      }
  ...
  router.post('/reports', async (req, res) => {
    try {
      ...
      if (!savedReport) {
        ...
        inMemoryStore.reports.push(newRep);
        savedReport = newRep;
      }
  ...
  router.delete('/reports/:id', async (req, res) => {
    try {
      ...
      const idx = inMemoryStore.reports.findIndex((r) => r._id === reportId && r.userId === userId);
      if (idx !== -1) {
